...
// ... (Asset importlar o'zgarishsiz qoladi)
...
// ── BUY MODAL for Oddiy Gifts with AI ──
const BuyOddiyModal = ({ gift, apiUser, onClose, onSuccess }) => {
  ...
  // AI Tabrik generatsiyasi
const generateAIGreeting = async () => {
  if (!aiPrompt.trim()) return;

  setAiLoading(true);

  // Ishonchli bepul modellar ro'yxati (bittasi ishlamasa keyingisi sinab ko'riladi)
  const FREE_MODELS = [
  ...
  for (const model of FREE_MODELS) {
    try {
      ...
      if (data.error) {
        console.warn(`${model} xato:`, data.error?.message);
      ...
      if (text) {
        ...
        setAiLoading(false);
        return; // muvaffaqiyat — chiqish
      }
    } catch (err) {
      console.warn(`${model} ulanish xatosi:`, err.message);
      lastError = err.message;
    }
  }

  // Hamma model muvaffaqiyatsiz bo'lsa
  console.error("Barcha modellar ishlamadi:", lastError);
  ...
  return (
    <div className="fixed inset-1 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-md bg-background rounded-b-3xl shadow-2xl z-10 overflow-y-auto pb-[50px]" style={{ maxHeight: "92vh" }}>
...
// ... (GiftsPage komponenti o'zgarishsiz qoladi)
